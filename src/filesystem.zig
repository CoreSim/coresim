        ...
        while (iterator.next()) |entry| {
            ...
            if (key.operation == operation) {
                ...
                if (matches) {
                    ...
                    self.operation_count.put(key.*, count + 1) catch |err| {
                        // Operation count tracking failed - log warning but continue
                        std.debug.print("Warning: Failed to track filesystem operation count: {}\n", .{err});
                    };
                    return error_type;
                }
            }
